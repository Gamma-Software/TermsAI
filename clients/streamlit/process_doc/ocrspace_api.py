...
class API:
    ...
    def _download_pdf(self, raw):
        if type(raw) == str:
            raise Exception(raw)
        if raw['IsErroredOnProcessing']:
            raise Exception(raw['ErrorMessage'][0])
        pdf_url = raw['SearchablePDFURL']
        return requests.get(pdf_url, stream=True)
    ...
    def ocr_file_to_pdf(self, fp):
        ...
        payload["iscreatesearchablepdf"] = True
        payload["issearchablepdfhidetextlayer"] = True
        with (open(fp, 'rb') if type(fp) == str else fp) as f:
            r = requests.post(
                self.endpoint,
                files={'filename': f},
        ...
        return self._download_pdf(r.json()).content
